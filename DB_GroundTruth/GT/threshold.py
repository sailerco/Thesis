...
columns = zsc_1.columns[1:]
row_headers = zsc_1.iloc[:, 0]
zsc = (pd.read_csv(f'../ClassifierOutput/bart.csv', header=None, skiprows=[0], index_col=None)
       .drop(columns=0))
zsc_th = zsc.applymap(lambda x: 1 if x >= threshold else 0)
zsc_th.columns= columns
zsc_th.index = row_headers
zsc_th.to_csv('threshold_09.csv', header=True, index=True)